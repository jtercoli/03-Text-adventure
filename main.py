...
def render(room,moves,points):
    ...
    print('\n\nIf you are dead or stuck, I would suggest typing "quit" :^)')
...
def getInput(verbs):
    ...
    response = input('\nWhat would you like to do? (You may go North, East, South, or West) ').strip().upper().split()
    if (len(response)):
...
def main():
    ...
    games = [
        (   "Dracula's Lair - Jadan Ercoli",          'game.json',    'START',    ['END'],    [])
